...
- **Python 3.10.13** (exact version required to match Docker containers)
...
1. Find your Docker bridge IP (you should verify):

   ```bash
   ip addr show docker0 | grep "inet\b" | awk '{print $2}' | cut -d/ -f1
   ```

2. Set the environment variable in your .env file (replace with your actual IP):
   ```bash
   SPARK_MASTER_IP="172.26.0.1"
   ```
